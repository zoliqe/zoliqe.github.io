...
function AudioFormatReader(ErrorCallback, DataReadyCallback) {
  ...
  if (typeof ErrorCallback !== 'function') throw new Error('AudioFormatReader: ErrorCallback must be specified');
  if (typeof DataReadyCallback !== 'function') throw new Error('AudioFormatReader: DataReadyCallback must be specified');
...
AudioFormatReader.prototype.PushData = function (data) {}; // Check if samples are available
...
AudioFormatReader.prototype.PurgeData = function () {}; // Force the reader to analyze his data
...
function CreateAudioFormatReader(MIME, ErrorCallback, DataReadyCallback) {
  if (typeof MIME !== "string") throw new Error('CreateAudioFormatReader: Invalid MIME-Type, must be string'); // Load format handler according to MIME-Type
  ...
  switch (MIME.replace(/\s/g, "")) {
    ...
    case "audio/mpa-robust":
      if (!CanDecodeTypes(new Array("audio/mpeg", "audio/MPA", "audio/mpa-robust"))) throw new Error('CreateAudioFormatReader: Browser can not decode specified MIME-Type (' + MIME + ')');
    ...
    case "audio/vorbis-config":
      if (!CanDecodeTypes(new Array("audio/ogg; codecs=vorbis", "audio/vorbis"))) throw new Error('CreateAudioFormatReader: Browser can not decode specified MIME-Type (' + MIME + ')');
    ...
    case "audio/ogg;codecs=opus":
      if (!CanDecodeTypes(new Array("audio/ogg; codecs=opus", "audio/opus"))) throw new Error('CreateAudioFormatReader: Browser can not decode specified MIME-Type (' + MIME + ')');
      return new AudioFormatReader_OGG(ErrorCallback, DataReadyCallback);
      break;
    ...
    */
    ...
    case "audio/wave":
    case "audio/x-wav":
      if (!CanDecodeTypes(new Array("audio/wav", "audio/wave"))) throw new Error('CreateAudioFormatReader: Browser can not decode specified MIME-Type (' + MIME + ')');
