...
function PCMAudioPlayer() {
  ...
  if (typeof AudioContext !== "undefined") this._SoundContext = new AudioContext();else if (typeof webkitAudioContext !== "undefined") this._SoundContext = new webkitAudioContext();else if (typeof mozAudioContext !== "undefined") this._SoundContext = new mozAudioContext();else throw new Error('PCMAudioPlayer: Browser does not support "AudioContext".'); // Set speed to default

  this._Speed = 1.0; // Prepare variable for scheduling times

  this._NextTime = 0.0; // Create gain node for volume control

  this._GainNode = this._SoundContext.createGain(); // Set volume to max

  this._GainNode.gain.value = 1.0; // Connect gain node to filters and to context
...
PCMAudioPlayer.prototype._StartOffset = 0.33; // Constants:
// ==========
...
PCMAudioPlayer.prototype._OffsetMax = PCMAudioPlayer.prototype._StartOffset + PCMAudioPlayer.prototype._OffsetVariance; // Pubic methods (external functions):
// ===================================
...
PCMAudioPlayer.prototype.SetVolume = function (Value) {
  ...
  if (Value <= 0.0) Value = 1e-20; // Cancel any scheduled ramps

  this._GainNode.gain.cancelScheduledValues(this._SoundContext.currentTime); // Change volume following a ramp (more userfriendly)
...
PCMAudioPlayer.prototype.MobileUnmute = function () {
  ...
  var audioBuffer = this._SoundContext.createBuffer(2, this._SoundContext.sampleRate, this._SoundContext.sampleRate); // Create new audio source for the buffer


  var SourceNode = this._SoundContext.createBufferSource(); // Make sure the node deletes itself after playback
  ...
  SourceNode.buffer = audioBuffer; // Connect the source to the gain node

  SourceNode.connect(this._GainNode); // Play source		
...
PCMAudioPlayer.prototype.DecodeAudioData = function (audioData, successCallback, errorCallback) {
  if (typeof errorCallback !== 'function') errorCallback = function () {}; // Call decoder
...
PCMAudioPlayer.prototype.PushBuffer = function (AudioBuffer) {
  ...
  if (this._NextTime + AudioBuffer.duration > this._SoundContext.currentTime) {
    var OffsetTime; // If the playback start time is in the past but the playback end time is in the future, we need to partially play the buffer.
    ...
    if (OffsetTime < AudioBuffer.duration) {
      ...
      SourceNode.loop = false; // Pass audio data to source
      ...
      if (this._VariSpeed) {
        ...
        if (this._NextTime - this._SoundContext.currentTime > this._OffsetMax) {
          ...
          if (this._Speed > 1.0 && this._NextTime - this._SoundContext.currentTime < this._StartOffset || this._Speed < 1.0 && this._NextTime - this._SoundContext.currentTime > this._StartOffset) {
