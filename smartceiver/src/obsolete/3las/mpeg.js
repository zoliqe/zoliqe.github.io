...
function AudioFormatReader_MPEG(ErrorCallback, DataReadyCallback) {
  AudioFormatReader.call(this, ErrorCallback, DataReadyCallback); // Dependencies:
  // =============
  // Create audio context

  if (typeof AudioContext !== "undefined") this._SoundContext = new AudioContext();else if (typeof webkitAudioContext !== "undefined") this._SoundContext = new webkitAudioContext();else if (typeof mozAudioContext !== "undefined") this._SoundContext = new mozAudioContext();else throw new Error('AudioFormatReader_MPEG: Browser does not support "AudioContext".'); // Internal variables:
  // ===================
  // Data buffer for "raw" framedata

  this._DataBuffer = new Uint8Array(0); // Array for individual frames

  this._Frames = new Array(); // Array for individual bunches of samples

  this._Samples = new Array(); // Indices that mark frame borders
...
let isAndroid = navigator.userAgent.toLowerCase().match('android'); // Adds a minimal ID3v2 tag to each frame

AudioFormatReader_MPEG.prototype._AddID3Tag = true; // Number of frames to decode together (keyword: byte-reservoir)
...
AudioFormatReader_MPEG.prototype._WindowSize = isAndroid ? 30 : 25; // Number of frames to use from one decoded window
// if (isAndroid && isFirefox)
...
AudioFormatReader_MPEG.prototype._UseFrames = isAndroid ? 5 : 2; // Constants:
// ==========
// MPEG versions - use [version]

AudioFormatReader_MPEG.prototype._mpeg_versions = new Array(25, 0, 2, 1); // Layers - use [layer]

AudioFormatReader_MPEG.prototype._mpeg_layers = new Array(0, 3, 2, 1); // Bitrates - use [version][layer][bitrate]

AudioFormatReader_MPEG.prototype._mpeg_bitrates = new Array(new Array( // Version 2.5
...
AudioFormatReader_MPEG.prototype._mpeg_srates = new Array(new Array(11025, 12000, 8000, 0), // MPEG 2.5
new Array(0, 0, 0, 0), // Reserved
new Array(22050, 24000, 16000, 0), // MPEG 2
new Array(44100, 48000, 32000, 0) // MPEG 1
); // Samples per frame - use [version][layer]

AudioFormatReader_MPEG.prototype._mpeg_frame_samples = new Array( //             Rsvd     3     2     1  < Layer  v Version
...
if (AudioFormatReader_MPEG.prototype._AddID3Tag) {
  AudioFormatReader_MPEG.prototype._ID3v2Tag = new Uint8Array(new Array(0x49, 0x44, 0x33, // File identifier: "ID3"
...
AudioFormatReader_MPEG.prototype.PushData = function (data) {
  ...
  this._DataBuffer = appendBuffer(this._DataBuffer, new Uint8Array(data)); // Try to extract frames
...
AudioFormatReader_MPEG.prototype.PopSamples = function () {
  if (this._Samples.length > 0) {
    ...
    var audioBuffer = this._Samples[0]; // Remove said bunch from the array

    this._Samples.shift(); // Hand it back to callee
...
AudioFormatReader_MPEG.prototype._ExtractAllFrames = function () {
  // Look for frames
  this._FindFrame(); // Repeat as long as we can extract frames


  while (this._CanExtractFrame()) {
    // Extract frame and push into array
    this._Frames.push(this._ExtractFrame()); // Check if we have enough frames to decode


    if (this._Frames.length >= this._WindowSize) {
      var SampleRates = new Array();
      var SampleCount = new Array(); // Sum the lengths of the individuall frames
      ...
      if (this._AddID3Tag) bufferlength += this._ID3v2Tag.length; // Create a buffer long enough to hold everything

      var windowbuffer = new Uint8Array(bufferlength);
      var offset = 0; // If needed, add ID3v2 tag to beginning of buffer
      ...
      for (var i = 0; i < this._UseFrames - 1; i++) this._Frames.shift(); // Push window to the decoder


      this._SoundContext.decodeAudioData(windowbuffer.buffer, function (buffer) {
        var srates = SampleRates;
        var scount = SampleCount;

        this.__decodeSuccess.bind(this)(buffer, srates, scount);
      }.bind(this), this.__decodeError.bind(this));
    } // Look for frames
...
AudioFormatReader_MPEG.prototype._FindFrame = function () {
  ...
  if (this._FrameStartIdx < 0) {
    var i = 0; // Make sure we don't exceed array bounds
  ...
  if (this._FrameStartIdx >= 0 && this._FrameEndIdx < 0) {
    ...
    if (this._FrameStartIdx + 2 < this._DataBuffer.length) {
      // Get header data
      // Version index
      var ver = (this._DataBuffer[this._FrameStartIdx + 1] & 0x18) >>> 3; // Layer index

      var lyr = (this._DataBuffer[this._FrameStartIdx + 1] & 0x06) >>> 1; // Padding? 0/1

      var pad = (this._DataBuffer[this._FrameStartIdx + 2] & 0x02) >>> 1; // Bitrate index

      var brx = (this._DataBuffer[this._FrameStartIdx + 2] & 0xf0) >>> 4; // SampRate index

      var srx = (this._DataBuffer[this._FrameStartIdx + 2] & 0x0c) >>> 2; // Resolve flags to real values
      ...
      var slot_size = this._mpeg_slot_size[lyr]; // In-between calculations

      var bps = samples / 8.0;
      var fsize = bps * bitrate / samprate + (pad == 1 ? slot_size : 0); // Truncate to integer

      var FrameSize = Math.floor(fsize); // Store number of samples and samplerate for frame

      this._FrameSamples = samples;
      this._FrameSampleRate = samprate; // Set end frame boundry
...
AudioFormatReader_MPEG.prototype._CanExtractFrame = function () {
  if (this._FrameStartIdx < 0 || this._FrameEndIdx < 0) return false;else if (this._FrameEndIdx < this._DataBuffer.length) return true;else return false;
...
AudioFormatReader_MPEG.prototype._ExtractFrame = function () {
  ...
  var framearray = this._DataBuffer.buffer.slice(this._FrameStartIdx, this._FrameEndIdx); // Remove frame from buffer


  if (this._FrameEndIdx + 1 < this._DataBuffer.length) this._DataBuffer = new Uint8Array(this._DataBuffer.buffer.slice(this._FrameEndIdx));else this._DataBuffer = new Uint8Array(0); // Reset Start/End indices

  this._FrameStartIdx = 0;
  this._FrameEndIdx = -1;
  return {
    'data': new Uint8Array(framearray),
    'samples': this._FrameSamples,
    'rate': this._FrameSampleRate
  };
...
AudioFormatReader_MPEG.prototype.__decodeSuccess = function (buffer, SampleRates, SampleCount) {
  ...
  */
  ...
  for (var i = 1; i < SampleCount.length - 1; i++) CalcTotalPlayTime += SampleCount[i] / SampleRates[i];

  CalcTotalPlayTime += SampleCount[SampleCount.length - 1] / SampleRates[SampleCount.length - 1] / 2.0; // Calculate the expected number of samples

  var CalcSampleCount = CalcTotalPlayTime * buffer.sampleRate; //console.log(CalcTotalPlayTime, buffer.duration);

  var DecoderOffset; // Check if we got the expected number of samples
  ...
  for (var i = 1; i < this._UseFrames - 1; i++) ExtractTimeSum += SampleCount[SampleCount.length - 1 - i] / SampleRates[SampleCount.length - 1 - i];
  ...
  var audioBuffer = this._SoundContext.createBuffer(buffer.numberOfChannels, ExtractSampleNum, buffer.sampleRate); // Fill buffer with the last part of the decoded frame leave out last granule


  for (var i = 0; i < buffer.numberOfChannels; i++) audioBuffer.getChannelData(i).set(buffer.getChannelData(i).subarray(buffer.length - OffsetRight + DecoderOffset - ExtractSampleNum, buffer.length - OffsetRight + DecoderOffset)); // Push samples into array


  this._Samples.push(audioBuffer); //this._Samples.push(buffer);
